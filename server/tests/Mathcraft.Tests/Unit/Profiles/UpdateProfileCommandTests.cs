using FluentAssertions;
using Mathcraft.Server.Common;
using Mathcraft.Server.Features.Profiles;
using Mathcraft.Server.Models;

namespace Mathcraft.Tests.Unit.Profiles;

public class UpdateProfileCommandTests
{
    private static readonly Guid AccountId = Guid.NewGuid();
    private static readonly Guid OtherAccountId = Guid.NewGuid();

    private static PlayerProfile MakeProfile(Guid accountId) => new()
    {
        Id = Guid.NewGuid(), FamilyAccountId = accountId,
        DisplayName = "Original", AvatarId = 1, Age = 8,
        CreatedAt = DateTime.UtcNow, UpdatedAt = DateTime.UtcNow
    };

    [Fact]
    public async Task Handle_ValidUpdate_ReturnsUpdatedProfile()
    {
        using var db = TestDbFactory.Create();
        var profile = MakeProfile(AccountId);
        db.PlayerProfiles.Add(profile);
        await db.SaveChangesAsync();

        var handler = new UpdateProfileCommandHandler(db);
        var result = await handler.Handle(new UpdateProfileCommand(AccountId, profile.Id, "Updated", null, null), default);

        result.Success.Should().BeTrue();
        result.Data!.DisplayName.Should().Be("Updated");
    }

    [Fact]
    public async Task Handle_WrongAccount_ReturnsForbidden()
    {
        using var db = TestDbFactory.Create();
        var profile = MakeProfile(AccountId);
        db.PlayerProfiles.Add(profile);
        await db.SaveChangesAsync();

        var handler = new UpdateProfileCommandHandler(db);
        var result = await handler.Handle(new UpdateProfileCommand(OtherAccountId, profile.Id, "Hack", null, null), default);

        result.Success.Should().BeFalse();
        result.ErrorCode.Should().Be(ErrorCode.Forbidden);
    }

    [Fact]
    public async Task Handle_NotFound_ReturnsNotFound()
    {
        using var db = TestDbFactory.Create();
        var handler = new UpdateProfileCommandHandler(db);

        var result = await handler.Handle(new UpdateProfileCommand(AccountId, Guid.NewGuid(), "X", null, null), default);

        result.Success.Should().BeFalse();
        result.ErrorCode.Should().Be(ErrorCode.NotFound);
    }
}
